variable "docker_compose_values" {
  type = object({
    ...
    RPC_docker_image       = string
    rpc_address                   = string
    chain_id                      = string
    rust_verification_service_url = string
  })
  default = {
    RPC_docker_image       = "RPC/RPC-polygon-supernets:4.1.8-prerelease-651fbf3e"
    postgres_host                 = "postgres"
    postgres_password             = "postgres"
    postgres_user                 = "postgres"
    rpc_address                   = "http://localhost:9632"
    chain_id                      = "93201"
    rust_verification_service_url = "https://sc-verifier.aws-k8s.RPC.com/"
  }
...
variable "path_docker_compose_files" {
  type        = string
  default     = "/opt/RPC"
  description = "Path for RPC files"
}
